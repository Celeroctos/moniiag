...
class CladrStreet extends MisActiveRecord {
    ...
    public function getRows($filters, $sidx = false, $sord = false, $start = false, $limit = false) {
        ...
        $streets = $connection->createCommand()
            ...
            ->from(CladrStreet::model()->tableName().' cst')
            ->leftJoin(CladrRegion::model()->tableName(). ' cr', 'cst.code_region = cr.code_cladr');
    ...
    public function getOne($id) {
        try {
            ...
            $street = $connection->createCommand()
                ...
                ->from(CladrStreet::model()->tableName().' cst')
                ->leftJoin(CladrRegion::model()->tableName().' cr', 'cr.code_cladr = cst.code_region')
                ->leftJoin(CladrDistrict::model()->tableName().' cd', 'cd.code_cladr = cst.code_district')
                ->leftJoin(CladrSettlement::model()->tableName().' cs', 'cs.code_cladr = cst.code_settlement')
                ->where('cst.id = :id
                            AND cd.id IN(SELECT cd2.id
                                     FROM '.CladrDistrict::model()->tableName().' cd2
                                     WHERE cd2.code_cladr = cd.code_cladr
                                        AND cd2.code_region = cr.code_cladr)
                            AND cs.id IN(SELECT cs2.id
                                     FROM '.CladrSettlement::model()->tableName().' cs2
