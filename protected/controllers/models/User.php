...
class User extends MisActiveRecord  {
    ...
    public function getRows($filters, $sidx = false, $sord = false, $start = false, $limit = false) {
        ...
        $users = $connection->createCommand()
            ->select('u.login, u.username, u.employee_id, u.role_id, u.id, CONCAT(d.last_name, \' \', d.first_name, \' \', d.middle_name) as employee, r.name as rolename')
            ->from('mis.users u')
            ->leftJoin('mis.doctors d', 'd.id = u.employee_id')
            ->leftJoin('mis.roles r', 'r.id = u.role_id');
